...
export class StoryblokCMSService extends BaseCMSService {
  ...
  async fetchPageBySlug(slug: string, locale: string): Promise<Page | null> {
    ...
    const response =
      await this.safeFetch<StoryblokResponse<StoryblokStory>>(url);
  ...
  private transformPage(story: StoryblokStory): Page {
    ...
    return {
      title: story.name,
      hero: content.hero
        ? this.transformHero(content.hero)
        : {
            title: story.name,
            text: "",
          },
          ...
          : [
              {
                ...
                link: {
                  href: this.parseStoryblokLink(
                    heroBlock.cta.link || heroBlock.cta.url,
                  ),
