...
export class ContentfulCMSService extends BaseCMSService {
  ...
  async fetchPageBySlug(slug: string, locale: string): Promise<Page | null> {
    ...
    const response = await this.safeFetch<ContentfulResponse<ContentfulPage>>(url)
  ...
  private transformPage(contentfulPage: ContentfulPage): Page {
    ...
    return {
      title: fields.title,
      hero: fields.hero ? this.transformHero(fields.hero) : {
        title: fields.title,
        text: '',
      },
  ...
  private transformHero(contentfulHero: any) {
    return {
      ...
      image: contentfulHero.image ? this.transformImage(contentfulHero.image) : undefined,
    ...
    switch (item.__component) {
      ...
      case 'content.image-text':
        return {
          ...
          image: item.fields.image ? this.transformImage(item.fields.image) : undefined,
          paragraph: item.fields.paragraph,
        }
      default:
        return null
  ...
  private transformImage(contentfulImage: ContentfulAsset) {
    ...
    return {
      ...
      height: 800,
      formats: { thumbnail: { url: file.url, ext: '', hash: '', mime: file.contentType, name: '', path: null, size: 0, width: 150, height: 100, sizeInBytes: 0 } },
